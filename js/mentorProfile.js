...
// --- Collect form data into object matching backend/mentors.js field names ---
function collectFormData() {
  const data = new FormData(form);
  ...
  return {
    name: data.get("name").trim(),
    surname: data.get("surname").trim(),
    birthday: data.get("birthDate"),          // было birthDate
    gender: data.get("gender"),
    country: data.get("country"),
    email: data.get("email").trim(),
    phoneNumber: data.get("phone").trim(),     // было phone
    Position: data.get("position").trim(),     // было position
    description: data.get("description").trim(),
    avatar: photoUrlInput.value.trim(),        // было photo
    Status: true,                              // новый ментор не заблокирован
    "Skil-1": data.get("skill1").trim(),
    "Skil-2": data.get("skill2").trim(),
    "Skil-3": data.get("skill3").trim(),
    "Skil-4": data.get("skill4").trim()
  };
}
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // возвращаемся на страницу со списком менторов,
    // где get() заново запросит список и покажет новую карточку
    setTimeout(() => {
